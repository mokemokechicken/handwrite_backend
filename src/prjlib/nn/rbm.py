...
class RBM(StatePersistent):
    ...
    def __init__(self, input=None, n_visible=784, n_hidden=500, \
        ...
        if params:
            W, hbias, vbias = self.deserialize(params, 3)
...
def test_rbm(learning_rate=0.1, training_epochs=15,
    ...
    rbm = RBM(input=x, n_visible=28 * 28,
              n_hidden=n_hidden, numpy_rng=rng, theano_rng=theano_rng, params=loaded_params)
    ...
    if saved_params_filename:
        save_params(saved_params_filename, rbm)


if __name__ == '__main__':
    import sys
    filename = len(sys.argv) > 1 and sys.argv[1] or None
    test_rbm(training_epochs=1, saved_params_filename=filename)
